...
const ServiceProviderReviews = () => {
  ...
  const [searchText, setSearchText] = useState(location.state?location.state:"");
  ...
  useEffect(() => {
    API({
      method: 'get',
      url: `partner/reviews/get-business-service-provider-reviews?count=${pageRecordCount}&page=${page}&searchText=${searchText}`,
    }).then((response) => {
...
const  handlePageChange = (e) => {
      e.preventDefault()
       API({
          method: 'get',
          url: `partner/reviews/get-business-service-provider-reviews?count=${pageRecordCount}&page=${e.currentTarget.getAttribute('data-page')}&searchText=${searchText}`,
        }).then((response) => {
            setcustomerList(response.reviews.map((client, index) =>client ))
            setTotalPages(Number(response?.totalPages))
            setCustomerCount(response?.customersCount)
...
const pagination=()=>{
 ...
 const handleRecordCountChange = (e) => {
   e.preventDefault() 
    API({
        method: 'get',
        url: `partner/reviews/get-business-service-provider-reviews?count=${e.currentTarget.value}&page=${page}&searchText=${searchText}`,
      }).then((response) => {
          setcustomerList(response.reviews.map((client, index) =>client ))
          setTotalPages(Number(response?.totalPages))
          setCustomerCount(response?.customersCount)
                   ...
                   <table class="is-hoverable w-full text-left">
                    <thead>
                      <tr>
                        ...
                        <th class="whitespace-nowrap bg-slate-200 px-4 py-3 font-semibold uppercase text-slate-800 dark:bg-navy-800 dark:text-navy-100 lg:px-5">Client Name</th>
                        <th class="whitespace-nowrap bg-slate-200 px-4 py-3 font-semibold uppercase text-slate-800 dark:bg-navy-800 dark:text-navy-100 lg:px-5">Provider Name</th>
                        ...
                        <th class="whitespace-nowrap bg-slate-200 px-4 py-3 font-semibold uppercase text-slate-800 dark:bg-navy-800 dark:text-navy-100 lg:px-5">Phone#</th>
                        <th class="whitespace-nowrap bg-slate-200 px-4 py-3 font-semibold uppercase text-slate-800 dark:bg-navy-800 dark:text-navy-100 lg:px-5">Gender</th>           
                        <th class="whitespace-nowrap bg-slate-200 px-4 py-3 font-semibold uppercase text-slate-800 dark:bg-navy-800 dark:text-navy-100 lg:px-5">Reviews</th>                        
                        <th class="whitespace-nowrap bg-slate-200 px-4 py-3 font-semibold uppercase text-slate-800 dark:bg-navy-800 dark:text-navy-100 lg:px-5">Registered Date</th>
                        <th class="whitespace-nowrap bg-slate-200 px-4 py-3 font-semibold uppercase text-slate-800 dark:bg-navy-800 dark:text-navy-100 lg:px-5">Bookings</th>
                    ...
                    <tbody>
                        {customerList.map((client, index) => ( 
                         <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                            ...
                            <td class="whitespace-nowrap px-4 py-3 sm:px-5">{client?.author?.firstname && (client?.author?.firstname +" "+ client?.author?.lastname) }</td>
                            <td class="whitespace-nowrap px-4 py-3 sm:px-5">{client?.subject?.firstname &&  (client?.subject?.firstname+" "+client?.subject?.lastname) }</td>
                            ...
                            <div x-data="usePopper({placement:'bottom-end',offset:4})" class="inline-flex">
                                ...
                                onClick={() => {
                                    if(document.getElementById("pooper-"+client?.id).classList.contains("show")){
                                        document.getElementById("pooper-"+client?.id).classList.remove("show")
                                    }else{
                                        document.getElementById("pooper-"+client?.id).classList.add("show")
                                    }
                                }}>
                                ...
                                <div id={ "pooper-" + client?.id } x-ref="popperRoot" class="popper-root">
                                 <div class="popper-box rounded-md border border-slate-150 bg-white py-1.5 font-inter dark:border-navy-500 dark:bg-navy-700">
                                    <ul>
                                     <li>
                                      <button onClick={()=>onViewe(client?.id)} class="flex h-8 items-center px-3 pr-8 font-medium tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100" id="editClosingDateModal" >View</button>
                                     </li>
                                    </ul> 
                                </div>
                               </div>
                    ...
                    {customerList?.length == 0 && (  <div class="m-2 text-center mt-4 pt-4">
                      ...
                      <div class="m-2 mb-3">
                        <p>Provider reviews will appear here</p>
                      </div>
                    </div>)}
